// df_master.js — DYNAMIC FOCUS "MASTER" engine, v5.9 (page notes, Clear Page, lean logs)
//
...
function renderNote() {
    // textedit responds to "set <string>" on inlet 0.
    // We route through outlet 0 with a "set <text>" message-list.
    // JS outlet(n, ...) sends a message to outlet n.
    // textedit inlet 0 accepts: set <string>
    // We use a dedicated outlet-3 for textedit set commands (outlet 2 = page sel).
    // BUT current patch only has outlets 0,1,2. We'll reuse outlet 0 with a special prefix
    // that the patch can route, or better: add outlet 3 for textedit.
    //
    // Simpler approach: textedit responds to inlet 0 message "set <text>".
    // JS can send to outlet 0 a list starting with "noteset" which a route object can
    // forward to textedit. But that requires patching.
    //
    // CLEANEST: add outlet 3 specifically for textedit set.
    // JS declares outlets=4, outlet(3, "set", note) → textedit inlet 0.
...
function handleCmd(a) {
    ...
    if (first === "pgname") {
        // pgname <text> — user edited the page name textedit
        // a = ["pgname", text] where text may be a symbol or first word
        // textedit sends full string as second arg
        var newName = (a.length > 1) ? (a[1] + "") : "";
        // If multiple words, join (arrayfromargs gives separate elements for each word)
        if (a.length > 2) {
            var parts = [];
        ...
        log("pgname: page=" + currentPage + " note=" + newName);
        return;
    }
    if (first === "clearpage") {
        // Clear all 16 slots on current page
