...
//
// Only non-empty slots are included. JSON has no spaces -> single Max symbol atom.
// pattr df_data (parameter_enable=1) saves value per device instance in .als.
//
// ANTI-CLOBBER RULE:
...
function pushState() {
    ...
    if (slots.length === 0) {
        ...
        log("pushState: " + slots.length + " slot(s) -> pattr (" + json.length + " chars)");
        _pushing = true;
        outlet(3, json);
        _pushing = false;
    }
...
function restoreSlots(a) {
    ...
    log("restoreSlots: received " + a.length + " atom(s), first=" + a[0]);

    if (a.length === 0) { _loading = false; return; }

    // Sentinel: int -1 (explicit clear) or old format value
    ...
    // Expect single JSON symbol starting with '{'
    var raw = "" + a[0];
    if (raw.charAt(0) !== "{") {
        log("restoreSlots: unexpected format (first='" + raw.charAt(0) + "') -- ignoring, clearing _loading");
        _loading = false;
        return;
    }
