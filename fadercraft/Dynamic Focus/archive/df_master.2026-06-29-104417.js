...
function setvalueof() {
    ...
    // With Blob parametertype, pattr may deliver the stored string as a single symbol atom
    // OR as a list of atoms split on spaces (if Max tokenises the JSON). We join all args
    // with a space to reconstruct the original JSON string in either case.
    var args = arrayfromargs(arguments);
    log("setvalueof: received " + args.length + " arg(s), first=" + (""+args[0]).substring(0,60));

    if (!args || args.length === 0) { _loading = false; return; }

    // Join all atoms — handles both single-atom symbol and tokenised list delivery.
    var raw = args.join(' ');
    ...
    if (raw.charAt(0) !== "{") {
        log("setvalueof: unexpected format (first char='" + raw.charAt(0) + "', len=" + raw.length + ") -- ignoring");
        _loading = false;
        return;
    }
...
function pushState() {
    ...
    var preview = getvalueof();
    log("pushState: notifyclients (" + slots.length + " slot(s)), getvalueof preview=" + (""+preview).substring(0,60));
    notifyclients();
}
