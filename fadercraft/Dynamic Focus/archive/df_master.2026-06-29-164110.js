// df_master.js — DYNAMIC FOCUS "MASTER" engine, v6.1 (overlay placeholder comment)
//
...
autowatch = 0;
inlets  = 3;
outlets = 5;  // 0=row, 1=value, 2=page sel, 3=textedit set, 4=placeholder comment text
...
// -- renderNote: textedit + overlay placeholder comment ---------------
// outlet 3 → textedit  (inlet 0, "set <text>"): shows actual note or empty
// outlet 4 → pg_name_ph comment (inlet 0, "set <text>"): shows "Page name" when note empty
function renderNote() {
    var note = NOTES[currentPage] || '';
    if (note !== '') {
        outlet(3, "set", note);   // single-word or multi-word name → textedit
        outlet(4, "set", "");     // hide placeholder comment
    } else {
        outlet(3, "set", "");     // clear textedit (keep field empty for editing)
        outlet(4, "set", "Page name");  // show overlay placeholder
    }
}
...
function handleCmd(a) {
    ...
    if (first === "pgname") {
        ...
        if (newName === "Page name") newName = "";  // sentinel: placeholder text treated as empty
