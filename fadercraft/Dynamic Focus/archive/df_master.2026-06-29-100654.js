...
//                   9 atoms per non-empty slot. sentinel -1 only on explicit user unmap-all.
//                   spaces encoded as __SP__ in both name_sym and path_sym.
//
// Persistence (v4.1, path-based, anti-clobber):
//   PUSH: outlet(3) -> t_sw trigger -> pattr df_data (list first) + storagewrite (bang second).
//         RULE: pushState() writes to pattr ONLY when SLOTS has real data OR _explicitClear is set.
//         Incidental calls with empty SLOTS (numbox init, render, etc) are silently skipped.
//   PULL: pattrstorage (@parameter_enable 1) restores -> pattr -> js inlet 3 -> restoreSlots() -> Task(800ms).
...
function initSlots() {
    // This runs at script load (once) and from restoreSlots() before populating saved data.
    // Should appear at most TWICE per session in console: on load and on restoreSlots().
...
// pushState -> pattr -> pattr echoes list back to js inlet 3 -> restoreSlots().
// _pushing=true makes restoreSlots return early without wiping SLOTS.
var _pushing = false;

// Block pushState during device init / restore phase.
// On .als load Live initializes live.numbox objects (parameter_initial_enable=1) before
// pattrstorage restore fires. Each numbox init -> handleCmd min/max -> pushState.
// Without this guard those calls would write sentinel -1 to pattr, overwriting saved data.
// Cleared at end of restoreSlots Task (800ms) or by bang() Task (100ms) if no restore.
var _loading = true;

// Anti-clobber: pushState() NEVER writes empty/sentinel to pattr unless this flag is true.
// Set to true only inside clearSlot() when it's a genuine user "unmap last slot" action.
// All incidental empty-SLOTS calls (numbox init, render feedback, etc.) are no-ops.
...
function bang() {
    ...
    renderCurrentPage();
    renderPageSel();
    // If no restoreSlots() fired (no saved data in .als), clear _loading so that
    // user interactions (mapcc, mapparam, min, max) can push state normally.
    // Deferred so any in-flight pattr restore (arriving just after bang) gets to
    // run its own Task path first.
...
function handleCmd(a) {
    ...
    if      (cmd === "mapcc")    armCC(absSlot);
        ...
        reapply(absSlot);
        // Only push if this slot actually has data (user could drag numbox on empty row).
...
function clearSlot(absSlot) {
    ...
    renderAbsSlot(absSlot);
    // Check if any slots remain after this clear.
    var anyLeft = false;
    for (var i = 0; i < NSLOTS; i++) {
        var b = SLOTS[i];
    ...
    if (!anyLeft) {
        // Last slot cleared: explicitly write sentinel so .als knows state is empty.
        _explicitClear = true;
    }
    pushState();
...
//
// Sentinel: -1 (int) written ONLY by explicit clear (_explicitClear=true).
//
// ANTI-CLOBBER RULE (v4.1):
//   pushState() skips writing to pattr if SLOTS has no real data AND _explicitClear is false.
//   This means incidental calls (numbox init, render feedback, page switch) cannot overwrite
//   saved state in pattr with an empty sentinel. Only genuine user actions produce output.
//
function pushState() {
    if (_loading) { return; }
    ...
    if (atoms.length === 0) {
        if (!_explicitClear) {
            // No real data AND not a deliberate clear: skip. Do NOT touch pattr.
            // This is the anti-clobber gate. Numbox init, page change, etc. all land here.
            log("pushState: SLOTS empty + not explicit clear -- skipped (pattr unchanged)");
            return;
        }
        // Explicit clear: user unmapped the last slot. Write sentinel.
        log("pushState: explicit clear -> sentinel -1 -> pattr");
        _pushing = true;
        outlet(3, -1);
        _pushing = false;
    } else {
        var nSlots = atoms.length / 9;
        log("pushState: " + nSlots + " slot(s) -> pattr (" + atoms.length + " atoms)");
        _pushing = true;
...
function restoreSlots(a) {
    ...
    log("restoreSlots: received " + a.length + " atoms (first: " + a[0] + ")");
    ...
    log("restoreSlots: populated " + (n / aps) + " slot(s), resolveAll in 800ms");
    ...
    // _loading cleared AFTER resolveAll so late numbox inits can't corrupt pattr.
