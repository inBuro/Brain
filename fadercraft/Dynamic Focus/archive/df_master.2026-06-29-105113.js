...
//
// Encoding: JSON.stringify -> char-code array (integers only).
// pattr @parameter_enable 1 (default float/list type) CAN store a list of integers.
// This avoids the "bad number" error that occurs when returning a string from getvalueof.
//
// getvalueof(): returns [0] sentinel for empty state, or array of charCodes of JSON string.
// setvalueof(...): receives the same integer list; reconstructs string via fromCharCode.
// notifyclients(): signals pattr to re-read getvalueof after any SLOTS change.
// Guards: _loading (no clobber during init), _explicitClear (no clobber on accidental empty).
//
function getvalueof() {
    var slots = buildSlotsArray();
    if (slots.length === 0) {
        log("getvalueof: empty state -> sentinel [0]");
        return [0];
    }
    ...
    log("getvalueof: " + codes.length + " char-codes for " + slots.length + " slot(s)");
    return codes;
}

function setvalueof() {
    // pattr middle outlet -> js inlet 0 -> Max calls setvalueof with the stored integer list.
    // args = array of integers (charCodes), or [0] sentinel for empty state.
    var args = arrayfromargs(arguments);
    log("setvalueof: received " + args.length + " number(s), first=" + args[0]);

    if (!args || args.length === 0) { _loading = false; return; }

    // Sentinel: single 0 means nothing was saved.
    if (args.length === 1 && (args[0] === 0 || args[0] === -1)) {
        log("setvalueof: sentinel " + args[0] + " -- nothing to restore, clearing _loading");
        _loading = false;
        return;
    }
    ...
    log("setvalueof: reconstructed string (" + raw.length + " chars), first 60: " + raw.substring(0, 60));

    if (raw.charAt(0) !== "{") {
        log("setvalueof: unexpected format (first char='" + raw.charAt(0) + "') -- ignoring");
        _loading = false;
        return;
    }
