...
function setupFocus() {
    ...
    renderPageSel();
    renderNote();
    // Placeholder comment sits on top of the name field; make it click-through so the
    // textedit underneath stays editable (otherwise the comment swallows the click).
    try { var _ph = this.patcher.getnamed("pg_name_ph"); if (_ph) _ph.ignoreclick = 1; } catch(e) { log("ignoreclick set failed: " + e); }
}
...
// -- renderNote: textedit content + overlay placeholder visibility ----
// outlet 3 → pg_name_edit textedit: set text content
// pg_name_ph comment visibility controlled via this.patcher.getnamed() .hidden
function renderNote() {
    var note = NOTES[currentPage] || '';
    if (note !== '') outlet(3, "set", note);  // show this page's name
    else             outlet(3, "clear");       // empty page → actually clear the field (set "" leaves stale text)
    var ph = this.patcher.getnamed("pg_name_ph");
    if (ph) ph.hidden = (note !== '');  // hide comment when name present, show when empty
}
...
function page(n) {
    ...
    // Defer the switch one tick so a textedit focus-loss name-commit from the SAME click
    // reaches NOTES[currentPage] (the page being left) before currentPage changes.
    var _pg = pg;
    var tk = new Task(function() { setPage(_pg); }, this);
    tk.schedule(0);
}

// Single-atom message "clearpage" from the Clear Page button. A bare symbol on a
// js inlet calls the function of that name directly (NOT list()), so this must be
// a top-level function — not a branch inside handleCmd.
function clearpage() {
...
function handleCmd(a) {
    ...
    if (first === "clearpage") { clearpage(); return; }
    ...
    if (first === "page") {
        ...
        if (isNaN(pg) || pg < 0 || pg >= NPAGES) return;
        var _pg = pg;  // defer (see page()): let focus-loss name-commit land on the old page first
        var tk = new Task(function() { setPage(_pg); }, this);
        tk.schedule(0);
        return;
    }
    if (first === "pgname") {
        // pgname <text> — real-time from textedit outlet 3 (per-keystroke)
        ...
        newName = newName.replace(/[\r\n]+/g, '');  // single line only — drop any newlines
        newName = newName.slice(0, 24);
        NOTES[currentPage] = newName;
        renderNote();          // immediately update placeholder visibility (also re-sets field → strips the newline)
        pushNote(currentPage); // persist immediately (like slots) — no debounce window to miss a quick save
        return;
    }
    var visRow = first;
...
function armCC(absSlot) {
    ...
    learnCCSlot = absSlot;
    _armCCTime = Date.now();  // start debounce window: ignore first CC for 300ms
