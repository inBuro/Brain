...
function loadDeviceParams() {
    ...
    try {
        ...
        for (var k in _paramCache) {
            count++;
            var diag_p = new LiveAPI(null, "id " + _paramCache[k]);
            var diag_min = diag_p.get("min") + "";
            var diag_max = diag_p.get("max") + "";
            log("DIAG param: name=" + k + " pid=" + _paramCache[k] + " min=" + diag_min + " max=" + diag_max);
        }
