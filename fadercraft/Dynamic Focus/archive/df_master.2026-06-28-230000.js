...
//   • Add / Mult flonums → post-scale the output fraction (Add offset, Mult scale factor)
...
//         cmd = mapcc | mapparam | clear | min <v> | max <v> | add <v> | mult <v>
...
autowatch = 1;
inlets  = 3;
outlets = 3;   // 0=full row, 1=value, 2=add/mult row
...
// OUTADD[r]  = additive offset applied after Min/Max window (default 0.0, range ±1)
// OUTMULT[r] = multiplicative scale applied after Min/Max window (default 1.0)
// Final frac = clamp01( (lo + p*(hi-lo)) * OUTMULT[r] + OUTADD[r] )
var SLOTS = [], OUTMIN = [], OUTMAX = [], OUTADD = [], OUTMULT = [];
function initSlots() {
    SLOTS = []; OUTMIN = []; OUTMAX = []; OUTADD = []; OUTMULT = [];
    for (var i = 0; i < NSLOTS; i++) {
        SLOTS.push(null); OUTMIN.push(0); OUTMAX.push(100);
        OUTADD.push(0.0); OUTMULT.push(1.0);
    }
}
initSlots();
...
function rowCommand(a) {
    ...
    else if (cmd == "add")    { OUTADD[r]  = clampFloat(a[2]);  reapply(r); }
    else if (cmd == "mult")   { OUTMULT[r] = clampFloat(a[2]);  reapply(r); }
}
function clampPct(x) { x = parseFloat(x); if (isNaN(x)) return 0; return Math.max(0, Math.min(100, x)); }
function clampFloat(x) { x = parseFloat(x); if (isNaN(x)) return 0; return x; }
...
// Then apply Mult (scale) and Add (offset), clamp to [0,1] before writing.
function drive(idx, b, v) {
    ...
    var frac = lo + p * (hi - lo);
    frac = frac * OUTMULT[idx] + OUTADD[idx];               // Add/Mult post-scale
    frac = Math.max(0, Math.min(1, frac));                   // clamp to [0,1]
...
function renderRow(r) {            // labels + min/max + mapped flag (× shows only when mapped) + arm-blink
    ...
    outlet(2, r, OUTADD[r], OUTMULT[r]);   // add/mult → bpatcher flonums
}
...
function setvalueof(v) {
    ...
    for (var k = 0; k < plain.length; k++) {
        ...
        OUTADD[d.i]  = (d.oadd  == null) ? 0.0 : d.oadd;
        OUTMULT[d.i] = (d.omult == null) ? 1.0 : d.omult;
    }
    resolveAll();
    renderAll();
}
