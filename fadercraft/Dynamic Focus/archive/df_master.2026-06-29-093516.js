...
function restoreSlots(a) {
    ...
    if (a.length === 1 && (a[0] === -1 || ("" + a[0]) === "empty" || ("" + a[0]) === "-1")) {
        log("restoreSlots: sentinel, nothing to restore");
        return;
    }
    // Old JSON string format (v2.x): ignore gracefully.
    if (a.length === 1 && ("" + a[0]).charAt(0) === "[") {
        log("restoreSlots: old JSON format, ignoring");
        return;
    }
    ...
    if (n % 9 !== 0) {
        if (n % 8 === 0) {
            log("restoreSlots: v3.x format (8 atoms/slot), path=null on restore -- target will not resolve");
            aps = 8;
        } else {
            // Try best fit
            if (n % 9 < n % 8) { aps = 9; n = n - (n % 9); }
            else                { aps = 8; n = n - (n % 8); }
            log("restoreSlots: truncating to " + n + " atoms (aps=" + aps + ")");
        }
    }
    if (n === 0) return;
    ...
    log("restoreSlots: loaded " + (n/aps) + " slots (" + aps + " atoms/slot), resolveAll in 800ms");

    // Defer resolveAll -- pattrstorage restores BEFORE live.thisdevice bang,
    // LiveAPI not yet available. 800ms gives Live time to finish device init.
