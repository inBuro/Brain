...
function initSlots() {
    ...
    // If you see "[DF-Master] initSlots" in console more than twice per session,
    // something unexpected is calling initSlots (check autowatch / js reload).
...
function pushState() {
    ...
    _pushing = true;
    if (atoms.length === 0) {
        // Only write sentinel if we know there truly are no mappings.
        // This should not overwrite real data - we only reach here when SLOTS is genuinely empty.
        log("pushState: 0 non-empty slots -> pattr (sentinel -1) [SLOTS=" + SLOTS.filter(function(s){ return s !== null; }).length + " non-null]");
        outlet(3, -1);
    } else {
