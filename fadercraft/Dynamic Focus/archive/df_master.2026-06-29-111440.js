...
// Cleared at end of restoreFromCodes Task (800ms) or by bang() Task (1500ms) if no restore.
var _loading = true;
...
// Feedback-loop guard: pattr left outlet echoes back whatever we send to its inlet.
// While outlet(3) is writing to pattr, restoreFromCodes on inlet 0 must be ignored.
var _saving = false;
...
function restoreFromCodes(args) {
    ...
    // _saving guard: pattr echoes left outlet whenever it receives data on its inlet (from
    // our outlet 3 during pushState). Skip restore during those echo-back calls.
    if (_saving) { log("restore: skipped (echo from pushState)"); return; }
...
// _saving guard prevents the pattr left-outlet echo (feedback loop) from triggering
// restoreFromCodes while we are writing to pattr.
...
function bang() {
    if (inlet !== 0) return;
    setupFocus();
    // Do NOT call renderCurrentPage() here -- if pattr restore is in flight (800ms Task),
    // rendering now would show empty buttons (SLOTS populated but targets not yet resolved).
    // renderCurrentPage is called by: restoreFromCodes Task (800ms) and onSelectedTrack observer.
    // If no restore is coming (empty set), clear _loading after 1500ms (> restore Task's 800ms)
    // so we don't prematurely unblock pushState before restore Task completes.
    var t = new Task(function() {
        if (_loading) {
            log("bang: no restore fired by 1500ms -- clearing _loading, rendering");
            _loading = false;
            renderCurrentPage();
            renderPageSel();
        }
