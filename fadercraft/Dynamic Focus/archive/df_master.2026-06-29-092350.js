...
//                  8 atoms per non-empty slot. sentinel -1 (int) when no slots mapped.
...
function armParam(absSlot) {
    ...
    // already clicked the parameter before pressing Map Param, the observer stays silent
    // and the mapping never fires. pollSelectedParam() covers that case.
    pollSelectedParam();
}

// Read live_set view selected_parameter right now and capture it into learnPSlot.
// Safe to call at any time; guards: learnPSlot < 0 → noop; pid <= 0 → noop.
...
// Shared capture logic used by both the observer callback and pollSelectedParam.
...
function onSelectedParam(args) {
    ...
    if (pid <= 0) return;
    // Delegate to shared capture logic (same path as pollSelectedParam).
    captureParam(learnPSlot, pid);
}
...
//
// Sentinel: -1 (int) when no slots are mapped (ensures pattr stores something non-nil, avoids symbol-as-method loop)
//
...
function pushState() {
    ...
    var nSlots = atoms.length / 8;
    _pushing = true;
    if (atoms.length === 0) {
        // Sentinel: numeric -1 (NOT a symbol) — prevents "js: no function empty" loop.
        // pattr receives -1, stores it, feeds back -1 on restore.
        // restoreSlots([-1]) hits the numeric-sentinel guard below and returns immediately.
        // pattr.restore in .amxd JSON is also set to [-1] to match.
...
function restoreSlots(a) {
    ...
    // During pushState(), pattr echoes atoms back to us synchronously.
    // _pushing=true means we are the source of this data — ignore the echo.
    if (_pushing) { return; }

    // Numeric sentinel -1 (or legacy symbol "empty") = nothing to restore.
    // This guard MUST use numeric check, not string comparison, to avoid symbol-as-method-name
    // issue: a bare symbol arriving at js inlet triggers "js: no function <symbol>" error loop.
    if (a.length === 1 && (a[0] === -1 || ("" + a[0]) === "empty" || ("" + a[0]) === "-1")) {
        log("restoreSlots: sentinel received, nothing to restore");
        return;
    }
