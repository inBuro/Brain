...
function initSlots() {
    ...
    for (var i = 0; i < NOTE_PAGES; i++) NOTES.push(defaultNote(i));
}
...
function restoreFromParams(triggerSource) {
    ...
    // Page notes — saved non-empty names override the "Mapping N" defaults
    for (var pg = 0; pg < NOTE_PAGES; pg++) {
        var chunks = [];
        for (var t = 0; t < NOTE_PARAMS; t++) chunks.push(getParam("df_pg" + pg + "_t" + t));
        var dn = decodeNote(chunks);
        NOTES[pg] = (dn !== '') ? dn : defaultNote(pg);
    }
...
function renderNote() {
    ...
    // pass words as separate atoms so a multi-word name ("Mapping 1") is NOT
    // sent as one quoted symbol (textedit would display the literal quotes).
    var args = [3, "set"];
    if (note.length) args = args.concat(note.split(" "));
...
function handleCmd(a) {
    ...
    if (first === "pgname") {
        ...
        // (valuemode 0). We feed it through [prepend pgname], so atoms are:
        //   ["pgname", "text", <word1>, <word2>, ...]  → drop the "text" token.
        var parts = [];
        for (var i = 1; i < a.length; i++) parts.push(a[i] + "");
        if (parts.length && parts[0] === "text") parts.shift();
        var raw = parts.join(" ");
        // Latin-only: keep printable ASCII (0x20-0x7E); drop Cyrillic/other so
        // the buffer never stores non-ASCII (which would persist as "?").
        var newName = "";
        for (var k = 0; k < raw.length; k++) {
            var cc = raw.charCodeAt(k);
            if (cc >= 32 && cc <= 126) newName += raw.charAt(k);
        }
        newName = newName.slice(0, 24);
        NOTES[currentPage] = newName;
        renderNote();   // reflect the cleaned (Latin-only) value back into the field
        // no renderNote() here: don't clobber the field the user is typing in
        schedulePushNote(currentPage);  // debounced persist
        return;
    }
    var visRow = first;
