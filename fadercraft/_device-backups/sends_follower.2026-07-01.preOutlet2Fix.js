...
function mapall() {
    ...
    // Read TIdx/DIdx/PIdx fresh from LiveAPI (AbletonMCP sets params but does not
    // reliably fire live.numbox output → in-memory tIdx/dIdx/pIdx stay stale).
    var devPath = "";
    try {
    ...
    for (var s = 0; s < NSLOTS; s++) {
        ...
        if (devPath) {
            ...
            path = slotPathFromIdx(s);  // fallback to in-memory
        }
        if (!path) continue;
        var id = 0;
        try {
