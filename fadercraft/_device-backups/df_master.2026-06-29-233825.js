...
// DYNAMIC FOCUS core (POSITION-based, O(1) per slot — no param scanning):
// take the captured path's device/param POSITION and re-point its track portion
// to the CURRENTLY SELECTED track. Follows the selection, 1 LiveAPI per slot.
// (Name-scan follow was ~3s PER SLOT in M4L — unusable; this is sub-ms per slot.)
function resolveAll() {
    ...
    for (var i = 0; i < NSLOTS; i++) {
        ...
        _n++;
        // "live_set tracks N ..." -> "live_set view selected_track ..." (follow).
        // return_tracks/master paths stay absolute (they're global, don't follow).
        var rel = b.path.replace(/^live_set tracks \d+ /, "live_set view selected_track ");
        var api = new LiveAPI(null, rel);
