// df_master.js — DYNAMIC FOCUS "MASTER" engine, v6.4-perf
// Perf optimizations (2026-06-30):
//   A. _apiCache: LiveAPI objects reused in getParam/setParam (no new LiveAPI per call)
//   B. restoreFromParams: lazy slot restore — read df_sN_meta first, pull p0/p1/p2 only if slot non-empty
//   C. loadDeviceParams batched via Task (16 params/tick) to avoid UI freeze on start
//   D. setupFocus: skip first resolveAll if slots not yet restored (_restoreDone gate)
//
...
var _LOG = false;   // master switch for console prints — false = clean Max window
function log(s) { if (_LOG) post("[DF-Master] " + s + "\n"); }

// -- LiveAPI parameter cache (A: cache BOTH pid AND api object) --------
// _paramCache[name] = pid  (number, for backward compat / retry checks)
// _apiCache[name]   = LiveAPI object (reused in getParam/setParam — no new LiveAPI per call)
var _paramCache = {};
var _apiCache   = {};
var _cacheReady = false;

// Batch size for param scan loop (opt C: spread over multiple ticks)
var _BATCH = 16;

// Internal: build cache from a flat id-list starting at index i0.
// Calls itself via Task until done, then invokes onDone().
function _loadBatch(params, i0, total_ref, found_ref, onDone) {
    var total = total_ref[0], found = found_ref[0];
    var limit = Math.min(i0 + _BATCH * 2, params.length - 1);  // step *2 because pairs [id, pid]
    for (var i = i0; i < limit; i++) {
        ...
        if (nm.indexOf("df_s") === 0 || nm === "df_ch" || nm.indexOf("df_pg") === 0) {
            _paramCache[nm] = pid;
            _apiCache[nm]   = p;   // A: keep the object, reuse later
            found++;
        }
        // don't hold objects for non-df params (p goes out of scope → GC)
    }
    total_ref[0] = total;
    found_ref[0] = found;
    ...
    if (limit >= params.length - 1) {
        ...
        // schedule next batch (opt C: yield to UI thread)
        var _next = limit;
        var t = new Task(function() {
            _loadBatch(params, _next, total_ref, found_ref, onDone);
        }, this);
        t.schedule(0);
    }
}

function loadDeviceParams(onDone) {
    _paramCache = {};
    _apiCache   = {};
    _cacheReady = false;
    try {
        ...
        _loadBatch(params, 0, [0], [0], onDone);
    } catch(e) {
...
function ensureCache() {
    // Note: ensureCache() can't trigger async load — callers in Task context are fine
    // because loadDeviceParams() is always called first in bang() Task chain.
    if (!_cacheReady) { log("ensureCache: cache not ready (may be mid-batch)"); }
}

// A: getParam reuses cached LiveAPI object — zero new LiveAPI allocations
function setParam(name, val) {
    var api = _apiCache[name];
    if (api === undefined) { log("setParam not found: " + name); return; }
    try { api.set("value", val); }
    catch(e) { log("setParam ERROR " + name + ": " + e); }
}

// A: getParam reuses cached LiveAPI object
function getParam(name) {
    var api = _apiCache[name];
    if (api === undefined) return 0;
    try { return parseFloat(api.get("value")); }
...
function _armRestoreObserver() {
    ...
    try {
        _restoreObs = new LiveAPI(function() {
            ...
            var curVal = 0;
            try { curVal = parseFloat((_apiCache["df_s0_meta"] || new LiveAPI(null, "id " + pid)).get("value")); } catch(e) {}
...
function _scheduleRetry() {
    ...
    if (_retryIdx >= _retryDelays.length) {
        ...
        if (_restoreObs) { _restoreObs.property = ""; _restoreObs = null; }
        _restoreDone = true;   // allow pushNote/pushSlot on fresh device (v6.4 fix preserved)
        return;
    }
...
function bang() {
    ...
    var t = new Task(function() {
        // C: loadDeviceParams now batched; setupFocus + observer arm happen AFTER cache built
        loadDeviceParams(function() {
            setupFocus();
            _armRestoreObserver();
            _scheduleRetry();
        });
...
function setupFocus() {
    ...
    // D: skip resolveAll here — slots not yet restored; second resolveAll fires in restoreFromParams Task
    // Only render static UI (page selector + notes placeholder); value rows will update after restore.
    renderCurrentPage();
    renderPageSel();
    renderNote();
}
...
function restoreFromParams(triggerSource) {
    ...
    // B: Lazy slot restore — read meta first, skip p0/p1/p2 for empty slots.
    // Safety: a slot with path but cc=null AND omin=0, omax=0 gives meta=0.
    // Guard: treat slot as non-empty if meta!=0 OR any of p0/p1/p2 != 0.
    var restoredCount = 0;
    for (var s = 0; s < PERSIST_SLOTS; s++) {
        var meta = getParam("df_s" + s + "_meta");

        // Quick empty check: meta=0 means cc=null + omin=0 + omax=0.
        // Safe to skip only if p0/p1/p2 are also 0 (path-only with omax=0 edge case).
        if (meta === 0) {
            // Rare edge-case check: path encoded but omax forced to 0 by user.
            // Read p0 only (cheapest); if 0, slot is definitely empty.
            var p0_check = getParam("df_s" + s + "_p0");
            if (p0_check === 0) continue;  // truly empty — skip p1/p2 too
            // p0 != 0 means there IS a path; proceed with full read
            var p0   = p0_check;
            var p1   = getParam("df_s" + s + "_p1");
            var p2   = getParam("df_s" + s + "_p2");
            var path_f = p0 + p1 * 1e7 + p2 * 1e14;
            var md    = decodeMeta(meta);   // cc=null, omin=0, omax=0
            var ppath = decodePath(path_f);
            if (!ppath) continue;
            SLOTS[s] = newSlot();
            SLOTS[s].path = ppath; SLOTS[s].name = resolveNameFromPath(ppath); analyzePath(SLOTS[s]);
            OUTMIN[s] = 0; OUTMAX[s] = 0;
            restoredCount++;
            log("PARAM RESTORE (omax=0 edge): slot=" + s + " path=" + ppath);
            continue;
        }

        // Normal path: meta != 0, slot is non-empty
        ...
        if (!has_cc && !has_path) continue;  // meta was e.g. just omin/omax defaults, path empty
        ...
        if (has_path) { SLOTS[s].path = ppath; SLOTS[s].name = resolveNameFromPath(ppath); analyzePath(SLOTS[s]); }
        OUTMIN[s] = md.omin;
        OUTMAX[s] = md.omax;
        restoredCount++;
    }
    ...
    // B: lazy notes — only read pages that have any non-zero chunk
    for (var pg = 0; pg < NOTE_PAGES; pg++) {
        // Peek at t0 first; if 0, page is likely empty (sentinel v=0 = empty chunk)
        var t0 = getParam("df_pg" + pg + "_t0");
        var t4 = getParam("df_pg" + pg + "_t" + NAME_CHUNKS);
        if (t0 === 0 && t4 === 0) {
            // Both first chunks of both fields are 0 → page is empty, use defaults
            NOTES[pg]  = defaultNote(pg);
            NOTES2[pg] = defaultNote(pg);
            continue;
        }
        var aC = [t0], bC = [t4];
        for (var ti = 1; ti < NAME_CHUNKS; ti++) {
            aC.push(getParam("df_pg" + pg + "_t" + ti));
            bC.push(getParam("df_pg" + pg + "_t" + (ti + NAME_CHUNKS)));
        }
    ...
    var t2 = new Task(function() {
        resolveAll();   // D: this is the ONE resolveAll that matters (slots populated now)
        for (var s = 0; s < PERSIST_SLOTS; s++) {
            var b = SLOTS[s];
...
function handleCmd(a) {
    ...
    else if (cmd === "clearparam") clearParam(absSlot);   // X on param → keep CC
    else if (cmd === "clearcc")    clearCC(absSlot);       // clear CC → keep param
    else if (cmd === "min") {
...
function captureParam(absSlot, pid) {
    ...
    // Ignore parameters that belong to DF Master itself (clicking the Map button
    // selects it as selected_parameter). Stay armed; wait for the real target.
    try {
        var selfPath = (new LiveAPI(null, "this_device")).unquotedpath;
        if (selfPath && selfPath.length && ppath.indexOf(selfPath) === 0) {
            log("captureParam: ignored self param (" + pname + ") -- still armed");
            return;   // do NOT clear learnPSlot
        }
    } catch (e) {}
    ...
    analyzePath(SLOTS[absSlot]);   // derive devName + paramIdx (or mixerTail) for follow
    learnPSlot = -1;
    log("captureParam: absSlot=" + absSlot + " name=" + pname + " path=" + ppath);
    resolveAll();
    var _b = SLOTS[absSlot];
    log("captureParam CHECK: clicked='" + pname + "' devName='" + (_b ? _b.devName : "?")
        + "' devOcc=" + (_b ? _b.devOcc : "?") + " paramIdx=" + (_b ? _b.paramIdx : "?")
        + " -> RESOLVED='" + (_b && _b.target ? (_b.target.get("name") + "") : "NULL")
        + "' path=" + (_b && _b.target ? _b.target.unquotedpath : "?"));
    renderAbsSlot(absSlot);
    pushSlot(absSlot);
}
...
function newSlot() {
    return { cc: null, ch: null, name: null, path: null, pmin: 0, pmax: 1,
             devName: null, paramIdx: -1, mixerTail: null, devOcc: 0,
...
// Clear only the PARAMETER target — keep the CC mapping on this slot.
function clearParam(absSlot) {
    if (learnPSlot === absSlot) learnPSlot = -1;   // cancel arm/learn FIRST (even on an empty slot)
    var b = SLOTS[absSlot];
    if (!b) { renderAbsSlot(absSlot); return; }     // armed-but-empty slot → re-render: stop blink, hide X
    b.name = null; b.path = null; b.devName = null; b.paramIdx = -1; b.mixerTail = null; b.devOcc = 0; b.target = null;
    if (b.cc == null) SLOTS[absSlot] = null;   // nothing left → empty slot
    _explicitClear = true;
    renderAbsSlot(absSlot);
    pushSlot(absSlot);
}
// Clear only the CC mapping — keep the parameter target on this slot.
function clearCC(absSlot) {
    var b = SLOTS[absSlot];
    if (!b) return;
    b.cc = null; b.ch = null;
    if (learnCCSlot === absSlot) learnCCSlot = -1;
    if (b.name == null) SLOTS[absSlot] = null;
    _explicitClear = true;
    renderAbsSlot(absSlot);
    pushSlot(absSlot);
}
...
// Parse the captured path into a track-independent target spec:
//   device param -> { devName, paramIdx }  (match device by NAME, param by index in it)
//   mixer param  -> { mixerTail }           (volume/pan/sends — positionally consistent)
//   return/master/other -> absolute (no follow)
// devName is resolved ONCE here (1 LiveAPI), not on every track change.
function analyzePath(b) {
    b.devName = null; b.paramIdx = -1; b.mixerTail = null; b.devOcc = 0;
    if (!b.path) return;
    var m = b.path.match(/^live_set tracks (\d+) devices (\d+) parameters (\d+)$/);
    if (m) {
        var trackN = m[1], devIdx = parseInt(m[2], 10);
        b.paramIdx = parseInt(m[3], 10);
        try {
            var d = new LiveAPI(null, "live_set tracks " + trackN + " devices " + devIdx);
            if (d && d.id > 0) b.devName = d.get("name") + "";
        } catch (e) {}
        // occurrence: how many same-named devices precede devIdx (1st/2nd/... EQ)
        if (b.devName != null) {
            try {
                var tr = new LiveAPI(null, "live_set tracks " + trackN);
                var devs = tr.get("devices"), occ = 0, di = 0;
                for (var k = 0; k + 1 < devs.length && di < devIdx; k += 2) {
                    if (devs[k] != "id") continue;
                    var dd = new LiveAPI(null, "id " + devs[k + 1]);
                    if ((dd.get("name") + "") == b.devName) occ++;
                    di++;
                }
                b.devOcc = occ;
            } catch (e) {}
    ...
    m = b.path.match(/^live_set tracks \d+ (mixer_device .+)$/);
    if (m) { b.mixerTail = m[1]; return; }
}

// DYNAMIC FOCUS core: re-target slots to the SELECTED track. Match the DEVICE by
// NAME (so a different rack in the same slot is NOT hit), then the param by index
// inside it. Scans only the track's DEVICES (few) once — never all parameters.
function resolveAll() {
    var _t0 = Date.now(), _n = 0;
    var t = new LiveAPI(null, "live_set view selected_track");
    var devs = t.get("devices");
    var devMap = {}, di = 0;                       // device name -> [indices] (all occurrences)
    for (var k = 0; k + 1 < devs.length; k += 2) {
        if (devs[k] != "id") continue;
        var d = new LiveAPI(null, "id " + devs[k + 1]);
        var nm = d.get("name") + "";
        if (!devMap[nm]) devMap[nm] = [];
        devMap[nm].push(di);
        di++;
    }
    for (var i = 0; i < NSLOTS; i++) {
        ...
        _n++;
        var tgt = null;
        if (b.devName != null && b.paramIdx >= 0) {
            var arr = devMap[b.devName];               // same-named devices on this track
            var occ = b.devOcc || 0;
            if (arr && arr.length > occ)               // pick the SAME occurrence (1st/2nd EQ...)
                tgt = "live_set view selected_track devices " + arr[occ] + " parameters " + b.paramIdx;
        } else if (b.mixerTail) {
            tgt = "live_set view selected_track " + b.mixerTail;
        } else {
            tgt = b.path;                          // absolute (return/master) — no follow
        }
        var api = tgt ? new LiveAPI(null, tgt) : null;
...
// PICKUP takeover: the parameter stays FROZEN until the incoming control sweeps
// THROUGH its current position, then engages — no abrupt jump. Re-armed per slot
// on every track change (resolveAll sets engaged=false), so you re-pick-up per track.
function drive(idx, b, v) {
    var nlo = OUTMIN[idx] / 100, nhi = OUTMAX[idx] / 100;
    if (!b.engaged) {
        var cur = normValue(b);                                   // param value, 0..1
        var tgtV = (nhi !== nlo) ? (cur - nlo) / (nhi - nlo) : 0;  // incoming value holding it
        tgtV = Math.max(0, Math.min(1, tgtV));
        if (b.lastIncoming >= 0) {
            var a = Math.min(b.lastIncoming, v), z = Math.max(b.lastIncoming, v);
            if (tgtV >= a - PICKUP_EPS && tgtV <= z + PICKUP_EPS) b.engaged = true;
        } else if (Math.abs(v - tgtV) < PICKUP_EPS) {
            b.engaged = true;
        }
        b.lastIncoming = v;
        if (!b.engaged) return;                                   // not reached yet — don't move
    }
    b.lastIncoming = v;
    b.cur = v;
    var frac = Math.max(0, Math.min(1, nlo + v * (nhi - nlo)));
    b.target.set("value", b.pmin + frac * (b.pmax - b.pmin));
}

function ccLabel(absSlot) {
    var b = SLOTS[absSlot];
    return (b && b.cc != null) ? ("CC " + b.cc) : "Learn CC";
}
