...
inlets  = 1;
outlets = 5;   // 0 = выход девайса ("max" <v>) → mm_sig → 8-слот мэппер;
               // 1 = меню + map-слоты + "barvis 0/1" (видимость numbox «for MIDI map»);
               // 2 = "knobset <0..1>"     — обновить БОЛЬШОЙ круг (plain dial, UI-зеркало);
               // 3 = "mididialset <0..1>" — обновить numbox midi_dial (Live-параметр 0..127).
               //
               // ДВА РЕЖИМА (выбор «Source»):
               //   • A/B/C/D: выход = уровень выбранного трек-send; большой круг ↔ send
               //     (двусторонне+гард). Numbox СПРЯТАН.
               //   • Manual: выход = РУЧНОЙ источник manualVal (numbox midi_dial, 0..127→0..1) →
               //     мэппер; сенд НЕ трогается. Большой круг ↔ numbox (двусторонне+гард).
               //     Numbox ПОКАЗАН.
               // Контролы: БОЛЬШОЙ круг (plain dial obj-3, parameter_enable:0) = визуал+мышь,
               // по MIDI не мапится; numbox midi_dial (live.numbox, Int 0..127, parameter_enable:1)
               // = ЕДИНСТВЕННЫЙ Live-параметр → Cmd+M → пользователь мапит на хардвер/MIDI.

var NSLOTS         = 8;    // число слотов маппера (как у return-версии)
var _sfPatcher     = this.patcher; // кэш ссылки (this.patcher нестабилен внутри callback)
...
// ---- РУЧНОЙ источник (режим Manual) ----------------------------------------
// В Manual numbox midi_dial (Int 0..127) = РУЧНОЙ источник модуляции: его значение
// масштабируется /127 → 0..1 и идёт в выход (outlet(0,"max",manualVal) → mm_sig → мэппер).
// Сенд НЕ трогается. Большой круг ↔ numbox синхронны (гард). manualVal — 0..1.
//
// ПЕРСИСТ manualVal: numbox midi_dial (live.numbox, parameter_enable:1) = Live-параметр →
// Live сохраняет/восстанавливает его значение в .als ПЕР-ИНСТАНС. На загрузке
// восстановленное значение приходит в JS через userval("midi", v) где v уже 0..1
// (пройдя /127 в патче). Кэшируем в lastSliderVal, засеваем manualVal на входе в Manual.
var manualVal      = 0.0;
var lastSliderVal  = -1;     // последнее значение numbox midi_dial (0..1 после /127); -1=ещё не видели
var barVisShown    = -1;     // последнее посланное состояние видимости numbox (0/1; -1=не слали)
...
function rebuildMenu() {
    ...
    returnCountSnap = n;
    // SOURCE PERSISTENCE: "clear" на umenu c Live-параметром может сбросить параметр
    // к initial (=1, Send A), что вызывает select(1) прямо в теле rebuildMenu().
    // Сохраняем selectedSend ДО clear и восстанавливаем ПОСЛЕ append-loop, чтобы
    // ни потенциальный reset параметра, ни select()-коллбэк не перебили нужное значение.
    var savedSel = selectedSend;
    outlet(1, "menu", "clear");
    outlet(1, "menu", "append", "Manual");     // item 0 = Manual (ручной источник; selectedSend=-1)
    for (var k = 0; k < n; k++) {
    ...
    // Восстанавливаем сохранённый выбор (отменяем возможный clobber от clear).
    if (n > 0 && savedSel >= n) savedSel = n - 1;  // кламп (return удалён)
...
function userval(src, v) {
    ...
    if (src === "midi") {
        midiShown     = val;
        lastSliderVal = val;   // КЭШ numbox: нужен для seed manualVal на restore
    } else {
        bigShown  = val;
    }
...
// ---- АВТО-МАППИНГ (TIdx/DIdx/PIdx/MapAll) ----------------------------------
// Параметры устройства (Track-версия):
//   [1..8]  Max[slot0..7]  — из multimap TargetMax
//   [9..16] Min[slot0..7]  — из multimap TargetMin
//   [19..26] DI1-DI8
//   [27]    MapAll
//   [28..35] PI1-PI8
//   [36..43] TI1-TI8
//
// TIdx: обычный трек = 0-base; Return A=-1, B=-2, C=-3, D=-4.
...
// "mapall" — триггер от sf_mapall live.text (через патч).
// Читает TIdx/DIdx/PIdx для каждого слота, строит путь, резолвит Live-object-id,
// отправляет id через outlet 4 → multimap_panel in1 → mm_ididin → live.remote~
// (тот же механизм, что у Return-версии).
function mapall() {
    for (var s = 0; s < NSLOTS; s++) {
        ...
        if (!id) continue;
        slotPath[s] = path;
        outlet(4, s, id);         // → multimap_panel in1 → mm_ididin → MapButton live.remote~
