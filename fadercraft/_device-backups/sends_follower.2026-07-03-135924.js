...
//
// PARAMETER LAYOUT  (43 LIVE params, 0-based index — Mode is a UI-only tab,
//   parameter_enable=0, so it is NOT counted; all indices below are the actual
//   Live parameter indices as read back via LiveAPI / AbletonMCP)
//   [0]     Device On
//   [1-8]   DI1–DI8       device index of target, default -1
//   [9]     MapAll        trigger: always reset 0 → 1 (never set 1 if already 1)
//   [10-17] PI1–PI8       parameter index within target device, default -1
//   [18-25] TI1–TI8       track index of target track (encoding: see TIDX below)
//   [26-33] Max0–Max7     upper bound 0-100 (%), default 100
//   [34-41] Min0–Min7     lower bound 0-100 (%), default 0
//   [42]    sfcmd         encoded min/max write: slot*1000000 + min*1000 + max
//
...
//     set_device_parameter(... parameter_index=9, value=0)
//     sleep(0.12s)
//     set_device_parameter(... parameter_index=9, value=1)
//
...
//   SF-Return=43  Reverb=33  Chorus-Ensemble=16  Echo=53  Phaser-Flanger=31
...
// bpatcher objects in multimap.maxpat are named varname="bpslot0".."bpslot7".
// applySlotRanges() enumerates them and sorts by visual presentation-Y (top->bottom)
// before pushing ranges, so a row is addressed by its physical position rather than
// by a varname->row assumption (which the runtime does not guarantee to match).
...
function mapall() {
    ...
    for (var s = 0; s < NSLOTS; s++) {
        ...
        if (devPath) {
            ...
            try { var pt = new LiveAPI(devPath + " parameters " + (18 + s)); if (pt && pt.id != 0) tidx = Math.round(pt.get("value")[0]); } catch(e) {}
            try { var pd = new LiveAPI(devPath + " parameters " + (1  + s)); if (pd && pd.id != 0) didx = Math.round(pd.get("value")[0]); } catch(e) {}
            try { var pp = new LiveAPI(devPath + " parameters " + (10 + s)); if (pp && pp.id != 0) pidx = Math.round(pp.get("value")[0]); } catch(e) {}
            if (didx >= 0 && pidx >= 0) {
...
// ---- MULTIMAP RANGES --------------------------------------------------
// applySlotRanges() — after MapAll reads Max/Min from API parameters [26-33]/[34-41]
// and pushes TargetMax/TargetMin into each MapButton row of the multimap.
//
// ADDRESSING BY VISUAL ROW (not by varname).  The binding channel lights up
// Macro N on the Nth physical row (top=1). The range channel must land on the
// SAME physical row so Min/Max line up with the label. We therefore do NOT trust
// getnamed("bpslot"+s) to imply "row s+1": the varname->row order the runtime
// resolves is not guaranteed to match the visual top->bottom order (this was the
// source of the scramble). Instead we enumerate the bpslot bpatchers, read each
// one's real presentation-Y from its box, sort ascending (top->bottom), and push
// slot s's Min AND Max together into the s-th visual row. This makes both channels
// identity regardless of varname/array ordering, and — because Min and Max go to
// the SAME resolved subpatcher in the same pass — removes any Min-vs-Max divergence.
function applySlotRanges() {
    ...
    for (var s = 0; s < NSLOTS; s++) {
        try {
            var pm = new LiveAPI(devPath + " parameters " + (26 + s));
            maxV[s] = (pm && pm.id != 0) ? pm.get("value")[0] : 100;
        } catch(e) { maxV[s] = 100; }
        try {
            var pn = new LiveAPI(devPath + " parameters " + (34 + s));
    ...
    try {
        ...
        // Collect the bpslot boxes together with their visual Y position.
        // rowY prefers presentation_rect (the panel is shown in presentation),
        // then falls back to patching rect. getboxattr returns [x,y,w,h].
        var slots = [];
        for (var s = 0; s < NSLOTS; s++) {
            var bp = mmSub.getnamed("bpslot" + s);
        ...
        // Sort top -> bottom by visual Y. This is the physical row order the user
        // sees and the order the binding channel (Macro labels) uses.
        slots.sort(function(a, b) { return a.y - b.y; });

        // Push slot s's range into the s-th visual row (0 = top).
        for (var r = 0; r < slots.length; r++) {
            var slotSub = slots[r].box.subpatcher();
            if (!slotSub) continue;
