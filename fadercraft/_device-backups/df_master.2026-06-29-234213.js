...
function newSlot() {
    return { cc: null, ch: null, name: null, path: null, pmin: 0, pmax: 1,
             devName: null, paramIdx: -1, mixerTail: null,
...
function analyzePath(b) {
    b.devName = null; b.paramIdx = -1; b.mixerTail = null;
    if (!b.path) return;
    var m = b.path.match(/^live_set tracks \d+ devices (\d+) parameters (\d+)$/);
    if (m) {
        b.paramIdx = parseInt(m[2], 10);
        var dpath = b.path.replace(/ parameters \d+$/, "");   // "live_set tracks N devices D"
        try { var d = new LiveAPI(null, dpath); if (d && d.id > 0) b.devName = d.get("name") + ""; } catch (e) {}
        return;
    }
...
function resolveAll() {
    ...
    for (var i = 0; i < NSLOTS; i++) {
        ...
        if (b.devName != null && b.paramIdx >= 0) {
            var D = devMap[b.devName];
            if (D !== undefined) tgt = "live_set view selected_track devices " + D + " parameters " + b.paramIdx;
        } else if (b.mixerTail) {
