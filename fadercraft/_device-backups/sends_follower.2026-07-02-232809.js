...
// Bang from qmetro 33 — value polling + cheap periodic target check.
function bang() {
    // Observers are the primary sync mechanism. In case they haven't
    // been set up yet on cold load (race), cheaply check the target every
    // RESYNC_MS: one detectReturnIndex + one getcount, no track scan.
    var t = now();
    ...
    // === sendRefs values are tracked via onAnySendChange observer (push) ===
    // Polling loop removed. bang() here only resyncs — the value itself arrives on event.
}
