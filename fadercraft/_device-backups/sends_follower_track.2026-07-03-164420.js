...
//   Return track N: EARLIER returns (0..N-1) can send to return N → N items.
//   (Live: return X can only send to returns X+1…, so only X < N can send to N.)
function liveReturnCount() {
    try {
        var liveSet = new LiveAPI("live_set");
        if (_hostIsReturn && _hostReturnIdx >= 0) {
            return _hostReturnIdx; // returns 0..(N-1) can send to us
        }
...
function rebuildMenu() {
    ...
    for (var k = 0; k < n; k++) {
        // Regular track: k → sendLetter(k) = A, B, C… (destination return)
        // Return track N: k → sendLetter(k) = A, B, C… (SOURCE return sending to us)
...
function buildRef() {
    ...
    // On a return track N: follow return[selectedSend]'s send TO our return (at index N).
    // Path: live_set return_tracks selectedSend mixer_device sends _hostReturnIdx
    // (return[selectedSend] → sends[N] = its send knob pointing at our return)
    // On a regular track: follow own send to return[selectedSend] as before.
    var path;
    if (_hostIsReturn && _hostReturnIdx >= 0) {
        path = "live_set return_tracks " + selectedSend + " mixer_device sends " + _hostReturnIdx;
    } else {
        path = "this_device canonical_parent mixer_device sends " + selectedSend;
    }
    try {
...
function init() {
    ...
    var safetyTask = new Task(function() { resync(true); }, this);
    safetyTask.schedule(1000);
...
// Device parameters (Track version) — verified live layout (0-based param index):
//   [1..8]   DI1-DI8   device index of target device
//   [9]      MapAll    trigger
//   [10..17] PI1-PI8   parameter index within target device
//   [18..25] TI1-TI8   track index of target track
//   [26..33] Max1-Max8 upper bound 0-100 (%)
//   [34..41] Min1-Min8 lower bound 0-100 (%)
//   [42]     MIDI Src  |  [43] send_menu
// (Slot block 1..41 is index-identical to SF-Return; "Mode"/Source is a UI tab
//  with parameter_enable=0, so it does NOT occupy a param index — no +1 shift.)
//
...
// Read a bpatcher box's visual Y position (top edge). The panel is displayed in
// presentation, so presentation_rect is authoritative; fall back to the patching
// rect if presentation is unavailable. For bpatchers getattr("presentation_rect")
// can return null, so getboxattr is used (it returns [x, y, w, h]).
function bpatcherRowY(bp) {
    var r;
    try {
        r = bp.getboxattr("presentation_rect");
        if (r && r.length >= 2 && r[1] !== null && !isNaN(r[1])) return r[1];
    } catch(e) {}
    try {
        r = bp.getboxattr("patching_rect");
        if (r && r.length >= 2 && r[1] !== null && !isNaN(r[1])) return r[1];
    } catch(e) {}
    try {
        r = bp.rect;                 // window-relative fallback
        if (r && r.length >= 2 && !isNaN(r[1])) return r[1];
    } catch(e) {}
    return 0;
}

// Enumerate the 8 bpslot bpatchers of the multimap panel and return them sorted
// top->bottom by visual Y. This is the SINGLE source of truth for "which box is
// physical row r" - used by BOTH the range channel (applySlotRanges) and the
// binding channel (mapall). We must NOT trust getnamed("bpslot"+s) to imply
// "row s+1": the varname->row order the M4L runtime resolves is not guaranteed
// to match the visual top->bottom order (that mismatch was the source of the
// scramble). Returns an array of the bpslot boxes, index r = physical row r.
function orderedSlotBoxes(mmSub) {
    var slots = [];
    for (var s = 0; s < NSLOTS; s++) {
        var bp = mmSub.getnamed("bpslot" + s);
        if (!bp) { post("orderedSlotBoxes: bpslot" + s + " not found\n"); continue; }
        slots.push({ box: bp, y: bpatcherRowY(bp) });
    }
    slots.sort(function(a, b) { return a.y - b.y; });
    var boxes = [];
    for (var r = 0; r < slots.length; r++) boxes.push(slots[r].box);
    return boxes;
}

// ---- MULTIMAP RANGES --------------------------------------------------
// applySlotRanges() reads Max/Min from API parameters [26-33]/[34-41] and pushes
// TargetMax/TargetMin into each MapButton row of the multimap.
//
// ADDRESSING BY VISUAL ROW (not by varname). The binding channel (mapall) writes
// slot r's target id onto the r-th physical row. The range channel must land on
// the SAME physical row so Min/Max line up with the binding. We therefore
// enumerate the bpslot bpatchers, sort ascending by visual Y (orderedSlotBoxes),
// and push slot r's Min AND Max together into the r-th visual row. Both channels
// become identity regardless of varname/array ordering, and - because Min and Max
// go to the SAME resolved subpatcher in the same pass - any Min-vs-Max divergence
...
function applySlotRanges() {
    ...
    try {
        ...
        // Boxes sorted top->bottom: index r = physical row r (0 = top).
        var boxes = orderedSlotBoxes(mmSub);
        for (var r = 0; r < boxes.length; r++) {
            var slotSub = boxes[r].subpatcher();
            if (!slotSub) continue;
            var tmax = slotSub.getnamed("TargetMax[7]");
            var tmin = slotSub.getnamed("TargetMin[7]");
            // Push Min AND Max together into the same resolved subpatcher (row r).
            if (tmax) tmax.message(maxV[r]);
            if (tmin) tmin.message(minV[r]);
        }
    } catch(e) {
...
// writes the id directly into each slot's "mb_map_id" message box inside the
// multimap_panel subpatcher (bpslot<r> -> mb_map_id -> live.remote~). This
// addresses panel slots by name, bypassing mm_idroute - no outlet routing used.
//
// ADDRESSING BY VISUAL ROW (not by varname). Same reasoning as applySlotRanges:
// getnamed("bpslot"+s) does NOT reliably map to physical row s at runtime, so we
// resolve every slot's id FIRST (by parameter index s), then push slot r's id
// into the r-th visual row via orderedSlotBoxes (sorted top->bottom by Y). This
// keeps the binding channel identity and mutually consistent with the range
// channel, which uses the exact same ordered-box list.
function mapall() {
    ...
    // Resolve the target id for each slot (by parameter index), independent of
    // panel row order. ids[s] = 0 means "no target for slot s".
    var ids = [];
    for (var s = 0; s < NSLOTS; s++) {
        ids[s] = 0;
    ...
    // Push slot r's id into the r-th visual row (0 = top), using the same
    // Y-sorted box list the range channel uses.
    if (mmSub) {
        var boxes = orderedSlotBoxes(mmSub);
        for (var r = 0; r < boxes.length; r++) {
            if (!ids[r]) continue;   // no target resolved for this slot
            try {
                var slotSub = boxes[r].subpatcher();
                var mmsg = slotSub ? slotSub.getnamed("mb_map_id") : null;
                post("[SF-Track] row" + r + " slotSub=" + (slotSub?"OK":"NULL") + " mmsg=" + (mmsg?"OK":"NULL") + " id=" + ids[r] + "\n");
                if (mmsg) mmsg.message(ids[r]);
            } catch(e) { post("[SF-Track] row" + r + " NAV ERROR: " + e + "\n"); }
        }
    }
