...
function captureTarget(slot, path) {
    ...
    // ВРЕМЕННАЯ ДИАГНОСТИКА exclude: что захвачено и попало ли в self-driven.
    var _cid = 0;
    try { var _c = new LiveAPI(path); _cid = (_c && _c.id != 0) ? parseInt(_c.id,10) : 0; } catch(e){}
    var _m = path.match(/mixer_device\s+sends\s+(\d+)\s*$/);
    post("EXCLDIAG capture slot=" + slot + " returnIndex=" + returnIndex +
         " path=[" + path + "] capturedId=" + _cid +
         " sendIdxInPath=" + (_m ? _m[1] : "none") +
         " selfDrivenId=" + selfDrivenIds[slot] +
         " (matchIdx=" + (_m && parseInt(_m[1],10)===returnIndex) + ")\n");
