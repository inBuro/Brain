...
var _bootMs        = 0;    // отметка загрузки: в окне загрузки restore "Manual"(0) → Send A (umenu parameter_initial не применяется к обычному umenu)
...
function init() {
    _bootMs = now();                            // открыть окно загрузки: restore Manual → Send A
    installObservers();
...
function select(menuIndex) {
    ...
    // На загрузке umenu восстанавливает значение параметра; для свежей вставки это
    // приходит как Manual(0), хотя дефолт должен быть Send A. В окне загрузки (1.2с)
    // перенаправляем Manual→A. Пользовательский клик Manual (после загрузки) работает.
    if (mi === 0 && (now() - _bootMs) < 1200) mi = 1;
    var prevSel = selectedSend;   // откуда уходим
    var newSel  = mi - 1;         // 0(None)→-1, 1→A(0), 2→B(1)…
    // Уход в Manual из реального сенда: засеять manualVal ТЕКУЩИМ значением сенда,
    // чтобы параметр приземлился туда, откуда уходил (без прыжка в 0). sendRef ещё
    // указывает на СТАРУЮ цель — читаем до buildRef().
    if (newSel < 0 && prevSel >= 0 && sendRef && sendRef.id != 0) {
