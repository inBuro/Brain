...
var _diagLastResult = -1;  // ВРЕМЕННО: предыдущий result (детект изменения для лога)
var _diagUntil      = 0;   // ВРЕМЕННО: timestamp, до которого открыто окно лога
...
function bang() {
    ...
    var result = 0.0;
    var _diagVals = [];            // ВРЕМЕННАЯ ДИАГНОСТИКА: сырые value сендов
    for (var i = 0; i < sendRefs.length; i++) {
        ...
        if (ref && ref.id != 0) {
            ...
            if (isSelfDrivenId(parseInt(ref.id, 10))) continue;
            var v = ref.get("value");
            if (v && v.length > 0) {
                var val = parseFloat(v[0]);
                _diagVals.push(val.toFixed(4));   // ВРЕМЕННО
                if (followMode === 1) {
    ...
    // ВРЕМЕННАЯ ДИАГНОСТИКА read-lag: печатаем сырые value + result + таймстамп
    // КАЖДЫЙ тик в окне 400мс после ЛЮБОГО изменения result. Так видно, падает
    // ли read-значение в 0 за ОДИН тик (мгновенно) или рампится вниз много тиков
    // (Live-side интерполяция чтения). Вне окна — тихо (чтобы не спамить 50/с).
    var _now = now();
    if (Math.abs(result - _diagLastResult) > 0.0001) {
        _diagUntil = _now + 400;              // изменилось — открыть окно лога
    }
    if (_now <= _diagUntil) {
        post("SFLAG t=" + _now + " result=" + result.toFixed(4) +
             " vals=[" + _diagVals.join(",") + "]\n");
    }
