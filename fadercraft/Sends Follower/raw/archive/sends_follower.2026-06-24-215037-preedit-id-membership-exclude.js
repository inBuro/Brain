...
// Является ли путь send'ом, кормящим ТЕКУЩУЮ watched-return-шину?
// Такой путь имеет вид "live_set tracks <i> mixer_device sends <returnIndex>".
// Возвращает id этого send-параметра (для исключения из агрегации), или 0.
function selfDrivenSendId(path) {
    if (!path || returnIndex < 0) return 0;
    // путь должен оканчиваться на "... mixer_device sends <returnIndex>"
    var m = path.match(/mixer_device\s+sends\s+(\d+)\s*$/);
    if (!m) return 0;                       // цель не send / не конец на sends N
    if (parseInt(m[1], 10) !== returnIndex) return 0; // send в ДРУГУЮ шину — не петля
    try {
...
// Пересчитать selfDrivenIds по всем слотам (после смены маппинга/индекса).
function recomputeSelfDriven() {
    for (var s = 0; s < NSLOTS; s++) {
        selfDrivenIds[s] = selfDrivenSendId(slotPath[s]);
    }
}

// Исключён ли этот send-id из агрегации (является self-driven целью слота)?
...
function captureTarget(slot, path) {
    ...
    slotRetry[slot] = 0;
    selfDrivenIds[slot] = selfDrivenSendId(path); // петля? исключить из агрегации
...
function restorepath() {
    ...
    slotRetry[slot] = 0;
    selfDrivenIds[slot] = selfDrivenSendId(slotPath[slot]); // петля? исключить из агрегации
    retryResolve(slot);
}
