...
var lastDiagAt     = 0;    // timestamp последней диагностической печати (раз в ~1с)
...
post("SFTRK: sends_follower_track.js loaded\n");
...
function buildRef() {
    sendRef = null;
    if (!onTrack) {
        post("SFTRK buildRef: not on a regular track -> N/A\n");
        return;
    }
    var path = "this_device canonical_parent mixer_device sends " + selectedSend;
    try {
        var api = new LiveAPI(path);
        var rid = api ? api.id : 0;
        post("SFTRK buildRef: path=[" + path + "] id=" + rid + "\n");
        if (api && rid != 0) {
            sendRef = api;
        }
    } catch (e) {
        sendRef = null;
        post("SFTRK buildRef ERROR: " + e + "\n");
    }
}
...
function bang() {
    ...
    // Диагностика: раз в ~1с печатаем прочитанное значение (подтвердить follow).
    if ((t - lastDiagAt) >= 1000) {
        lastDiagAt = t;
        post("SFTRK bang: send " + selectedSend + " value=" + result + "\n");
    }
