...
inlets  = 1;
outlets = 4;   // 0 = значение слежения ("max" <v>); 1 = управление меню + map-слотами;
               // 2 = "knobset <0..1>"    — обновить БОЛЬШОЙ круг (plain dial, UI-зеркало);
               // 3 = "mididialset <0..1>" — обновить маленькую live.dial (MIDI-параметр).
               //
               // ГИБРИД двух контролов одного send'а:
               //   • БОЛЬШОЙ круг (plain dial obj-3, parameter_enable:0) = визуал + мышь;
               //     по MIDI НЕ мапится (plain dial — не Live-параметр).
               //   • маленькая live.dial (midi_dial, parameter_enable:1) = ЕДИНСТВЕННЫЙ
               //     Live-параметр → её пользователь мапит на хардвер/MIDI.
               // Оба шлют движение в один хендлер userval (запись в send); JS — единый
               // рулевой: при любом изменении приводит ОБА контрола к значению send.
...
var midiShown      = -1;     // последнее значение на маленькой live.dial (mididialset)
...
// Пишем 1:1 в выбранный send. Источник уже стоит на val → метим его *Shown=val
// (его не толкаем назад); ВТОРОЙ контрол подтянет следующий bang() к send.
// Шкала: оба контрола 0..1 == диапазон send value (0..1), масштаб не нужен — кламп.
function userval(src, v) {
    ...
    if (isNaN(val)) return;
    val = clamp01(val);
    if (selectedSend < 0) return;              // None — контролы инертны, не пишем
    ...
    if (src === "midi") midiShown = val;       // источник уже на val — не дёргаем его
    else                bigShown  = val;
    try {
...
function bang() {
    ...
    // --- гибрид: подтянуть ОБА контрола под текущий send -----------------------
    // Эхо-гард: если только что писали в send от контрола и read ≈ lastWritten —
    // это наше эхо, снимаем гард (контрол-источник уже на месте, второй догонит
    // через рассинхрон ниже). Иначе — обновляем КАЖДЫЙ контрол независимо, только
    // когда ЕГО показанное значение реально расходится с send'ом на EPS:
    //   • контрол, который сам сдвинул send, имеет *Shown == send → его не дёргаем;
    //   • ВТОРОЙ контрол расходится → подтянется (рвёт пинг-понг между ручками);
    //   • внешняя правка send (микшер/автоматизация) → расходятся ОБА → оба едут.
    if (lastWritten >= 0 && Math.abs(result - lastWritten) <= KNOB_EPS) {
        lastWritten = -1;             // эхо поглощено, снимаем гард
    }
    if (Math.abs(result - bigShown) > KNOB_EPS) {
        bigShown = result;
        outlet(2, "knobset", result);     // → БОЛЬШОЙ круг (prepend set, тихо)
    }
    if (Math.abs(result - midiShown) > KNOB_EPS) {
        midiShown = result;
        outlet(3, "mididialset", result); // → маленькая live.dial (set, тихо)
    }
}
