...
// parameter_type:2) как Live-параметр. При переносе девайса на ДРУГОЙ трек
// live.remote~ теряет цель (ID пропал), но Map[N]-тогглы остаются в состоянии
// «замаплено» (filled/yellow) — кнопки показывают «замаплено», хотя никакой
// реальной цели нет.
// FIX: при смене canonical_parent (= девайс переехал на другой трек) → сбросить
// все Map[N]-параметры в 0 через LiveAPI (this_device parameters N set value 0),
// отфильтровав по shortname "Map". live.remote~ уже не имеет цели, Map[N]=0
// возвращает кнопки в состояние «Map me».
// ИНВАРИАНТ (same-track reload): _hostTrackId = 0 на старте JS (каждая загрузка
// заново инициализирует все переменные). При init() записываем текущий ID трека.
// Нет сохранённого ID → нет сравнения → нет ложного сброса.
// ТОЛЬКО Track-девайс: Return-устройство цепляет параметры ЛЮБОГО трека (кросс-
// трек), поэтому переезд на другой return-трек НЕ инвалидирует существующие
// mapping-цели — сброс для Return НЕ применяется.
var _hostTrackId   = 0;    // Live object ID хост-трека; 0 = неизвестен (до init)
...
function resetMultimapSlots() {
    try {
        ...
        // (или если кэш не совпадает по размеру — пересобираем при странной смене).
        if (!_mapParamIdxs) {
            _mapParamIdxs = [];
            for (var i = 0; i < n; i++) {
                var papi = new LiveAPI("this_device parameters " + i);
                if (!papi || papi.id == 0) continue;
                var nm = papi.get("name");      // возвращает shortname (проверено в DF Master)
                if (nm && String(nm[0]) === "Map") {
                    _mapParamIdxs.push(i);
                }
            }
        }
        // Записать 0 в каждый Map-параметр
        for (var j = 0; j < _mapParamIdxs.length; j++) {
            try {
                var mp = new LiveAPI("this_device parameters " + _mapParamIdxs[j]);
...
function rebuildMenu() {
    ...
    // (Используется в resync() для детектирования смены трека.)
    if (_hostTrackId === 0) {
    ...
    try {
        ...
        if (nowOnTrack && _hostTrackId !== 0) {
            ...
            if (newId !== 0 && newId !== _hostTrackId) {
                ...
                _mapParamIdxs  = null;    // инвалидируем кэш индексов (параметры те же, но перепроверим)
                resetMultimapSlots();
