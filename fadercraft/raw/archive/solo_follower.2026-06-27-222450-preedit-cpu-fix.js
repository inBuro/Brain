...
function forceOwnSolo(v) {
    ...
    try {
        ...
        if (lastChangedExternalId > 0 && lastChangedExternalId !== ownTrackId) {
            var view = new LiveAPI("live_set view");
            if (isValidApi(view)) {
                view.set("selected_track", "id " + lastChangedExternalId);
            }
        }
    } catch (e) {}
}
