...
> ✅ **`page` и `hold` — независимые оси (подтверждено 2026-06-02).** CC49 и CC28 — это **разные** переменные, не «одно и то же с разным типом триггера»: `page` и `hold` входят в формулу самостоятельно. Подтверждено байтами — в `1.syx` CC28 (`0x1C`) и CC49 (`0x31`) лежат отдельными дескрипторами.
