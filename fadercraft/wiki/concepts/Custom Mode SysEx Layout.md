...
В наших файлах: `60` (\`) = no label, `64` (d) = стандартная текстовая метка (UNDO, REDO, Kick), `66` (f) и `68` (h) = разные цвета подписей для track-names типа "Melody 1", "Perc 3". В mode 15 встречается ещё `6a` (j) — четвёртый цвет/стиль подписи, использован для метки `"Cue Volume"`.
