...
CC47 — это **один физический CC**, читаемый по значению и по слою:
- в [[Instruments Layer]]: **mode-report `10·N`** (descriptor `0x2F` эмитит при активации инструмент-мода — это же значение и запускает cross-transit) + **`127`** = возврат. Instrument-listen и cross-transit — это **один и тот же поток CC47**, не два разных назначения (подтверждено байтами: `2F 0A` / `2F 14`).
- в [[Mixer Layer]]: **momentary-кнопка `mixer_bank`** (значения 1/2).

Дискриминация работает потому, что значения не пересекаются (bank = 1/2; cross-transit = кратные 10 и 127) **и** контроллер всегда ровно в одном слое. Подробный разбор «один CC47, два слоя» — в [[Instruments Layer]].
