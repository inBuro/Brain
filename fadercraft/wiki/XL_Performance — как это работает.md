...
Launch Control XL MK3 хранит пользовательские custom-modes во внутренней памяти; патч занимает **15 слотов** (README v1.5 говорит «до 16», хардварный максимум 15/16 не подтверждён — см. [[Custom Modes Model]]); стандартно режимы выбираются руками. `XL_Performance.amxd` превращает контроллер в **режимный сервер**: режимы выбираются программно через CC30/ch7, между ними организованы переходы с памятью предыдущего состояния, а собственный трек устройства всегда «слышен» вместе с любым внешним соло благодаря встроенному [[Solo Follower]]'у — это **неотъемлемая часть** устройства, без неё router-трек XL_Performance глохнет в момент любого внешнего solo и весь сценарий рифф-перформанса ломается (source: XL_Performance.README.md). Custom-modes 1–10 отданы под инструменты, 11–14 — под микшер, 15 — Cue/prelisten volume.
