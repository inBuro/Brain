...
## 2026-08-18 — `/sf-updates` построен, `learn_deck` удалён, ARCHITECTURE.md вычищен

Три хвоста, всплывшие при дневной сверке, закрыты и задеплоены (`inBuro/fadercraft-landing@2bdc593`, прод проверен curl'ом).

**Роут под кнопку «New Version».** `api/versions.json` выдавал обоим устройствам Sends Follower URL `/sends-follower/updates`, который отдавал 404. Безобидно ровно до бампа версии: при `latest` = 1.0 кнопка не загорается, но в момент выхода 1.1 каждый нажавший покупатель уехал бы в пустоту. Страница построена как **`/sf-updates`** — плоское имя в ряд с `/xl-updates` и `/df-updates`, а не вложенное. Содержимое changelog — та же строка «Initial release», что уже лежала в манифесте; новой копи не изобреталось. Манифест перенацелен, а `/sends-follower/updates` оставлен живым как 301: этот URL уже носят в себе проданные девайсы.

Роут зарегистрирован во **всех** списках, где обязан быть (правило «ни одного enum 2-из-3»): `App.tsx` (lazy-карта и заголовки), `main.tsx` `ROUTE_LOADER`, `seo-meta.mjs` (chunk-карта и запись маршрута), `prerender.mjs`, `sitemap-lastmod.mjs`, `sitemap.xml`. В футере рядом с Sends Follower появился бейдж `v 1.0` со ссылкой на страницу — зеркально Control XL, иначе новый роут ушёл бы в прод сиротой.

**Побочный дефект, найденный при обходе этих списков:** `/df-updates` пререндерился, но не имел записи в `ROUTE_LOADER` — то есть гидратировался ровно через ту чёрную вспышку, ради устранения которой эта карта и существует. Починено там же.

**`learn_deck` удалён.** Запись висела в `api/versions.json` с `url` на корень сайта — остаток отменённого «Mapping Deck». Подтверждено основателем: продукта не будет.

**`Dynamic Focus/ARCHITECTURE.md` вычищен** — из начала файла убран случайно надиктованный фрагмент про папки агентов (266 символов), не относящийся к архитектуре.

Проверено на собранном `dist`: страница рендерит запись v1.0 и Gumroad-CTA, футер линкует её с хаба, `/sends-follower` и `/control-xl`, ошибок страницы нет. На проде: `/sf-updates` = 200, `/sends-follower/updates` = 301 → `/sf-updates`, манифест отдаёт четыре продуктовые записи с живыми URL, sitemap = 12 URL и пересабмичен (`204`).
