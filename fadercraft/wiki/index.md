...
- [[roadmap]] — живой checklist прогресса: **Phase 0 (Control XL) 121/121 = 100%**, Phase 1 (post-launch дистрибуция), **Phase 2 — Sends Follower 20/24 (~83%)**, Phase 3 — Dynamic Focus (запущен). Сверен с живым продом 2026-08-18 (задача E1 из [[seo-audit-2026-08-16]]); до сверки отставал на полтора месяца и держал «БЛОКЕР» на давно состоявшемся деплое. Обновляется при каждом крупном шаге.
...
- [[seo-audit-2026-08-16]] — внешний аудит индексации `fadercraft.com` (по индексу Google + почте, без доступа к проду/репо) + очередь задач для агента. Находки: сайт перестроен (хаб + `/control-xl`, `/sends-follower`, `/dynamic-focus`), продукт **Dynamic Focus $19 отсутствует в вики**, в индексе висит **preview-деплой Cloudflare Pages** (дубль сайта, протухшая сборка), судьба `/free-custom-modes` под вопросом, roadmap расходится с реальностью по 4 пунктам. Содержит вердикт по вопросу основателя о переводе страниц на FR/DE/en-GB (**нет, не сейчас**). **Дополнен 2026-08-18** (§8): разбор обвала брендовой выдачи и AI-показов (технической поломки не было), два закрытых тормоза краула — генерируемый из git `lastmod` и канонические ссылки на гайды вместо `?from=`-вариантов; выгрузка GSC за 3 месяца и итоговый вердикт по локализации (**нет** — 382 показа и 7 кликов за квартал, из них 48% Таиланд).
...
- [[dynamic-focus]] — продукт #3: per-track focus/banking для абсолютных контроллеров ($19, v1.2, запущен). Почему устройств два (`DF Input` + `DF Slot`) и как из ограничения Ableton «MIDI Effect только на MIDI-треках» вырастает вся архитектура; доказательная база из майнинга, позиционирование (хук = видимость, оффер = per-track), состояние продукта.
