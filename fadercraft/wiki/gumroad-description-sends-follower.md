...
> **Status (2026-06-26):** copy locked against the [[sends-follower-vo-script|VO script]]. Decisions in: licensing = one key / 3 activations (mirrors Control XL, unenforced); bundle = two devices + Readme, **no demo .als** (Kirill will flag if that changes); WHO IT'S FOR section **cut**. Remaining gate before publish: SF demo video + a final Vale/LanguageTool pass.
...
· Two devices: follow one track's send, or the activity of an entire return bus
· Drop it on a track or a return, map it, and play
· Max for Live device — works in any Live 11 or 12 project
...
• Readme — setup, plus which stock Ableton effects to map and what to move
...
Gumroad emails a download link whenever a new version ships. No subscription. The update check is built into the device. Stuck, or found a bug? support@fadercraft.com — replies within 48 hours.
...
> **Note:** Gumroad's "Custom message" field is hard-limited (~500 chars) — this is the minimal version that fits. Full WHAT'S INSIDE / UPDATES / SUPPORT detail lives in the bundled Readme and the product description, not here.
...
Full guide is in the Readme. Support: support@fadercraft.com
...
• Sends Follower – Track — follows one track's send (Send A/B/C/D, or Manual)
• Sends Follower – Return — follows an entire return bus (Peak or Total)
• Readme — 1-minute setup + which stock Ableton effects to map

Open the Readme first — drop the device on a return, map it, and play.
