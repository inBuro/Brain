...
function msg_int(v) {
    ...
    if (!initialized) {
        ...
        scheduleApply(currentDesiredSolo(), lastSoloedExternalId);
    }
}
...
function scheduleApply(desiredSolo, selectedId) {
...
function applyStateDeferred() {
    ...
    if (pendingDesiredSolo === 1 && pendingSelectedId > 0) {
        selectTrackById(pendingSelectedId);
    }
}
