...
**Summary**: JS-скрипт `solo_follower.js`, держащий собственный трек устройства заSOLOенным, пока заSOLOен любой другой трек, удерживающий пользовательское выделение (`selected_track`) на месте даже при программном `set("solo")` и старающийся уместить в видимом viewport одновременно засоленный трек и трек, выделенный пользователем.
