...
Как номера логических режимов 1..14 в `XL_Performance` отображаются на значения **CC30/ch7**, которые принимает LCXL для смены custom-mode.

## Mixer (modes 11–14)

Состояние трёх флагов:
...
Дыра 10..17 нужна, чтобы зарезервировать диапазон 24..27 под mixer (выше) и не ломать поток при cross-mode transit. См. [[../entities/CC47 Cross-Mode Transit]].

Входной overlay CC (по умолчанию 49) принимает значения **10·N** для N=1..10 (т.е. 10, 20, …, 100). Значения 0 и 127, а также любые «не-кратные 10» — фильтруются как мусор.
