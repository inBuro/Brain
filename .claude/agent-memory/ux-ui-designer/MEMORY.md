...
## Когда писать в `questions.md`

Не на каждый вопрос. Inline-вопросы решаем в чате — это норма (80% случаев).

**Write-triggers:**

1. **Пользователь явно откладывает** («к этому вернёмся», «потом», «park it», «не сейчас», «оставим на подумать») → фиксируй сейчас же со статусом 🟡 DEBT.
2. **Junction-point audit** — при смене темы / конце сессии / завершении заметного work unit (закоммитил фичу) / перед `/context-save` или явной автокомпрессией → кратко просканируй чат с прошлого junction: какие вопросы остались без ответа, какие наблюдения проронил мимо, какие отсрочки висели. Пакетом перенеси в `questions.md`. **Один раз, не по чуть-чуть.**

**Формат записи:** статус-emoji + короткое название + симптом → гипотеза → что нужно для разрешения. Группируй по категории внутри файла.

**Закрытие:** вопрос разрешён → перенеси в раздел `✅ Closed (history)` с датой и кратким описанием решения. Не удаляй — оставь для контекста.
