...
- [sends-follower.md](sends-follower.md) — SendsFollower.amxd: пути, dlst dire-структура (подтверждена), version-check + Follow Mode (Peak/Total) + self-healing return-index (observers) + **Map button (SF-direct map на любой Live-параметр через 2-й `live.remote~`, BYPASS LFO; стоковая иконка multimap-unmap.svg по имени, не вшита; pattr-persist пути; frozen 2026-06-18)**. Девайс **frozen + самодостаточен** (md5 `6f156eab…`, 68498 B, 82/85): `sends_follower.js` (`c63475…`, 22368 B, observers+map) И `sf_version_check.js` (`a5d905fc`, 3106 B) вшиты, внешних `.js` для работы НЕ нужно. ⚠️ Path-B prefix = `d[:48]` (incl mx@c sub-header), НЕ `d[:0x20]`. Правка в User Library `~/Music/Ableton/User Library/Max Devices/SendsFollower.amxd`, архив `~/Brain/Sends Follower/raw/archive/`. Проектная wiki = `~/Brain/Sends Follower/wiki/` (docs English, Vale). Читать перед любой правкой SendsFollower.
