...
- **Где редактировать Control XL (РАЗВОРОТ 2026-06-06 (8)):** основной редактируемый артефакт = **User Library** `.amxd` (`~/Music/Ableton/User Library/Max Devices/Control XL.amxd`). Прежнее правило «edit canon in project repo, User Library pristine/deploy-only» — **ОТМЕНЕНО**. Пропагация дальше (репо-эталон, бандл-слоты 3–6, zip) — **только по явной команде** «копируй дальше»/«пропагируй». Детали: [controlxl-project-map.md](controlxl-project-map.md), [xl-performance.md](xl-performance.md).
  - ⚠️ Browser Load (CC51) — В АКТИВНОЙ ОТЛАДКЕ, не финализирован. UL теперь **`0fc224e442c273e4aecfeb1e11fe25dc`** (164262 B, 272/411, собран **Путём B** из базы `ccd230c8`). THREADING-фикс: `deferlow` (`bl_defer`) между триггерами и js; `bl_ctlin (ctlin 51, any ch)/bl_ui_btn → bl_defer → bl_js (js fc_bload2.js)`. js `fc_bload2.js` без Task, `doLoad()` напрямую (рядом на диске, НЕ во freeze). **Инцидент порчи `095885b6`:** Путь A прошёл size/suffix-валидацию, но Max не парсил → теперь ОБЯЗАТЕЛЕН реальный JSON-парсер на извлечённом patcher (`json.load`+`jq`) + Путь B (без padding-run). Детали — [xl-performance.md](xl-performance.md), [amxd-format.md](amxd-format.md); НЕ ведём журнал версий ([feedback-iterative-no-versionlog.md](feedback-iterative-no-versionlog.md)). Бандлы 3–6 (`44aa142b`) и слот 1 (`63d95bbe`) — пропагация по команде.
