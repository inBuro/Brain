...
- [sends-follower.md](sends-follower.md) — SendsFollower.amxd: пути, dlst dire-структура (подтверждена), version-check + **Follow Mode (Max↔Sum, live.tab)** фичи, rack round-trip, LFO=сток. Девайс **frozen + самодостаточен** (md5 `7ca595b4…`, 42090 B, 65/65, rebuilt 2026-06-18 после инцидента «Live разморозил/потёр 725a06ea до архива»): `sends_follower.js` (5808 B) И `sf_version_check.js` вшиты, внешних `.js` для работы НЕ нужно. Правка в User Library `~/Music/Ableton/User Library/Max Devices/SendsFollower.amxd`, архив `~/Brain/Sends Follower/raw/archive/`. Проектная wiki = `~/Brain/Sends Follower/wiki/` (docs English, Vale). Читать перед любой правкой SendsFollower.
