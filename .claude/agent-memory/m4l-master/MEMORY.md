...
- [sends-follower.md](sends-follower.md) — SendsFollower.amxd: пути, dlst dire-структура (подтверждена), version-check фича (зеркало Control XL «New Version»), rack round-trip, LFO=сток. Девайс теперь **frozen + самодостаточен** (md5 `b5286b33…`, 37444 B, 61/61): `sends_follower.js` И `sf_version_check.js` вшиты, внешних `.js` для работы НЕ нужно. Правка в User Library `~/Music/Ableton/User Library/Max Devices/SendsFollower.amxd`, архив `~/Brain/Sends Follower/raw/archive/`. Проектная wiki = `~/Brain/Sends Follower/wiki/` (docs English, Vale). Читать перед любой правкой SendsFollower.
