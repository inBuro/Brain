...
- **Sends Follower** — отдельный standalone-проект (НЕ Instrument Follower), wiki = `~/Brain/Sends Follower/wiki/` (entities/concepts/index/log), девайс = `raw/SendsFollower.amxd`. Return-track audio FX: читает sends всех треков в этот return через LiveAPI (JS `sends_follower.js`), выдаёт МАКС → `live.remote~` на `devices 1 parameters 5` + шины `---max_send`/`---max_send_percent`. ⚠️ JS НЕ вшит и НЕ на диске рядом (есть только Archive-копия) → девайс упадёт `can't find file`. Контейнер unfrozen, JSON @0x32. Детали — в проектной wiki, не дублировать.
