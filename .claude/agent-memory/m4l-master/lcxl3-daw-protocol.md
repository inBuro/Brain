...
- **Relative (endless) энкодеры:** существуют ТОЛЬКО в DAW mode (per-row, pivot `0x40`=64, `>64` CW / `<64` CCW). В **custom mode НЕДОСТУПНЫ** — подтверждено техподдержкой Novation (feature-request, не реализовано). Для custom-mode `.syx` флага absolute/relative нет.
  - **Команды подтверждены (2026-07-13):** per-row relative toggle = CC69/CC72/CC73 ch7 val=127 — извлечено из декомпилированного Live 12 скрипта `Launch_Control_XL_3/midi.py` (`SET_RELATIVE_ENCODER_MODES = ((182,69,127),(182,72,127),(182,73,127))`, github.com/gluon/AbletonLive12_MIDIRemoteScripts). Там же: connect-handshake `F0 00 20 29 02 15 02 7F F7` (вход в DAW mode) и `(182,71,127)` = enable encoder touch output. НЕ подтверждено: действуют ли toggles, когда девайс в Custom Mode (скрипт шлёт их после handshake, в DAW mode). Артефакты для теста: `EncoderRelativeMonitor.amxd` + `EncoderRelativeTest.amxd` в `~/Brain/fadercraft/Control XL/raw/`. Детали: [encoder-relative-research.md](encoder-relative-research.md).
