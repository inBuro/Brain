...
- `$channel_type` / `$is_bounce` / `$entry_*` live on the **`sessions`** table, NOT on `events` — `properties.$channel_type` on events returns null (not in events taxonomy). For source classification off `events`, derive from `properties.$referring_domain` + `properties.utm_source`.
- **`$virt_is_bot` is unreliable for landing bots.** Verified 2026-06-12: it flags only Gumroad webhook pings (`purchase`) + `$session_summary_ready`, NOT the link-preview/scraper bots that hit `/` after a Reddit URL drop. Roll your own bot tag for these: `browser IS NULL/'None'` OR (`$viewport_height=$screen_height` AND single pageview, no CTA). That manual rule classified 15 of 60 lifetime sessions as bots.
- Lifetime bot/human SQL pattern (whole-history audit, owner excluded): CTE per session computing pv/dl/vid + vh_eq_sh + browser, then tag `is_bot` and `source` (reddit if utm/ref reddit; direct if `$direct`/null; internal_ref if ref=fadercraft.com = an inter-page navigation, NOT a new source), then GROUP BY source,is_bot summing events. Used for the 2026-06-12 go/no-go demand read.
- **HogQL function name traps (DataVisualizationNode / HogQLQuery):** `toTimezone` → must be `toTimeZone` (capital Z). `toInt32`/`toInt64` → must be `toInt`. Errors are sequential — fix the first, re-run to see the next.
- **HogQL GROUP BY + SELECT:** any `properties.*` reference in SELECT that is NOT in GROUP BY must be wrapped in an aggregate (`any()`, `max()`, etc.). Even if you GROUP BY `properties.video_id`, referencing it bare in a `multiIf()` in SELECT triggers "not under aggregate function" error. Fix: use `any(properties.video_id)` in SELECT; value is deterministic because GROUP BY guarantees one value per group.
