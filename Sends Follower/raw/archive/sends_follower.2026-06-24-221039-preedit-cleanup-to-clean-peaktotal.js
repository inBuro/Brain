...
var _exDiagAt      = 0;    // ВРЕМЕННО: timestamp последней exclude-диагностики
...
// Анти-feedback (схема «выход на свой вход»): id целей слотов, которые сами
// являются send'ом, кормящим отслеживаемую return-шину. Такой send девайс
// драйвит через свой маппер → если включить его в агрегацию, образуется
// положительная обратная связь (замапленный send держит max/sum и не идёт вниз).
// Поэтому в bang() ИСКЛЮЧАЕМ эти id из агрегации (по одному на слот).
// Сравнение по id (надёжнее строкового пути). Пусто = обычный случай, агрегация
// считает все сенды как раньше.
var selfDrivenIds  = [];   // id замапленных целей-сендов watched-шины (на слот; 0 = нет)
for (var _d = 0; _d < NSLOTS; _d++) selfDrivenIds[_d] = 0;
...
function buildRefs(returnIdx) {
    ...
    // После пересборки sendRefs пересчитать, какие mapped-цели — self-driven
    // сенды watched-шины (returnIndex мог смениться).
    recomputeSelfDriven();
}

// Резолвит LiveAPI-id ЛЮБОЙ захваченной map-цели (по её пути), или 0.
// БЕЗ индексной арифметики (прежний `sends <N> === returnIndex` ломался при
// group-треках / нестандартном порядке — index/id-mismatch). Исключение из
// агрегации теперь по ID-MEMBERSHIP: цель исключается ТОЛЬКО если её id реально
// есть среди агрегируемых sendRefs (т.е. она кормит watched-шину). Цель на
// другую шину/параметр просто не совпадёт ни с одним sendRef.id и не исключится.
function targetId(path) {
    if (!path) return 0;
    try {
        var t = new LiveAPI(path);
        return (t && t.id != 0) ? parseInt(t.id, 10) : 0;
    } catch (e) {
        return 0;
    }
}

// Есть ли хоть одна захваченная map-цель (непустой slotPath)?
function hasMappedTargets() {
    for (var s = 0; s < NSLOTS; s++) {
        if (slotPath[s]) return true;
    }
    return false;
}

// Пересчитать selfDrivenIds = id всех захваченных целей слотов.
// Источник истины id — тот же LiveAPI(path), что у sendRefs → id сравнимы.
function recomputeSelfDriven() {
    for (var s = 0; s < NSLOTS; s++) {
        selfDrivenIds[s] = targetId(slotPath[s]);
    }
}

// Исключён ли этот send-id из агрегации? = является ли он id какой-либо
// захваченной map-цели. Совпадение возможно ТОЛЬКО когда цель сама — один из
// агрегируемых sendRefs (кормит watched-шину) → петля рвётся. Цель на другую
// шину имеет id, которого нет в sendRefs, поэтому сюда не попадёт.
function isSelfDrivenId(id) {
    if (!id) return false;
    for (var s = 0; s < NSLOTS; s++) {
        if (selfDrivenIds[s] && selfDrivenIds[s] === id) return true;
    }
    return false;
}
...
// Колбэк смены выбранного параметра. Захватываем цель только если взведён слот;
// обычные клики пользователя по параметрам вне арма игнорируем.
...
function captureTarget(slot, path) {
    ...
    slotRetry[slot] = 0;
    selfDrivenIds[slot] = targetId(path); // петля? исключить из агрегации
    // ВРЕМЕННАЯ ДИАГНОСТИКА exclude: что захвачено + текущие sendRef id для сравнения.
    var _cid = 0;
    try { var _c = new LiveAPI(path); _cid = (_c && _c.id != 0) ? parseInt(_c.id,10) : 0; } catch(e){}
    var _sr = [];
    for (var _i = 0; _i < sendRefs.length; _i++) {
        if (sendRefs[_i] && sendRefs[_i].id != 0) _sr.push(parseInt(sendRefs[_i].id,10));
    }
    post("EXCLDIAG CAPTURE slot=" + slot + " path=[" + path + "]" +
         " capturedId=" + _cid + " sendRefs=[" + _sr.join(",") + "]" +
         " => in_sendRefs=" + (_sr.indexOf(_cid) >= 0) + "\n");
...
function restorepath() {
    ...
    if (toks.length === 0) { slotPath[slot] = ""; selfDrivenIds[slot] = 0; return; }
    slotPath[slot] = toks.join(" ");
    emitTargetName(slot, slotPath[slot]);
    slotRetry[slot] = 0;
    selfDrivenIds[slot] = targetId(slotPath[slot]); // петля? исключить из агрегации
    retryResolve(slot);
}
...
function unmap(slot) {
    ...
    slotPath[slot]  = "";
    slotRetry[slot] = 0;
    selfDrivenIds[slot] = 0;              // цель снята — больше не исключаем из агрегации
...
function bang() {
    ...
    // id-membership: держим selfDrivenIds актуальными против ТЕКУЩИХ sendRefs.
    // Дёшево пересчитываем только когда есть хоть один захваченный слот (иначе
    // обычный случай — пустые пути → ничего не резолвим, агрегат как раньше).
    if (hasMappedTargets()) recomputeSelfDriven();

    // Аккумулятор: при Peak — максимум, при Total — сумма с клампом к 1.0.
    var result = 0.0;
    var _exDiag = (now() - _exDiagAt) >= 700;   // ВРЕМЕННО: окно лога раз в 700мс
    var _inList = [], _exList = [];
    for (var i = 0; i < sendRefs.length; i++) {
        var ref = sendRefs[i];
        if (ref && ref.id != 0) {
            // Анти-feedback: пропускаем send, который девайс сам же драйвит
            // через свой маппер (его id попал в selfDrivenIds). Иначе он держал
            // бы max/sum и не давал агрегату идти вниз. Пусто = ничего не режем.
            if (isSelfDrivenId(parseInt(ref.id, 10))) {
                if (_exDiag) _exList.push(parseInt(ref.id,10));
                continue;
            }
            var v = ref.get("value");
            if (v && v.length > 0) {
                var val = parseFloat(v[0]);
                if (_exDiag) _inList.push(parseInt(ref.id,10) + "=" + val.toFixed(3));
                if (followMode === 1) {
    ...
    // ВРЕМЕННАЯ ДИАГНОСТИКА exclude: раз в 700мс печатаем returnIndex, selfDrivenIds,
    // какие send-id ВОШЛИ в агрегат vs ИСКЛЮЧЕНЫ, итоговый result. Видно, реально ли
    // mapped «Send A» в selfDrivenIds и выпал ли он из суммы/пика.
    if (_exDiag) {
        _exDiagAt = now();
        // slotPath-состояние: если все пусты → captureTarget НЕ вызывался (маппинг
        // идёт мимо JS через native live.map в маппере) = корень неработающего exclude.
        var _sp = [];
        for (var _k = 0; _k < NSLOTS; _k++) _sp.push(slotPath[_k] ? "set" : "-");
        post("EXCLDIAG bang ret=" + returnIndex + " mode=" + (followMode?"Total":"Peak") +
             " hasMapped=" + hasMappedTargets() +
             " slotPath=[" + _sp.join(",") + "]" +
             " selfDriven=[" + selfDrivenIds.join(",") + "]" +
             " excluded=[" + _exList.join(",") + "]" +
             " in=[" + _inList.join(",") + "]" +
