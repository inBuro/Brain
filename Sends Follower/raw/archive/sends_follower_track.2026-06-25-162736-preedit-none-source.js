...
function select(k) {
    ...
    selectedSend = idx;
    buildRef();
    // Новый send выбран — форсируем ресинк ручки под его текущее значение:
    // сбрасываем гард/последнее-показанное, чтобы СЛЕДУЮЩИЙ bang сразу толкнул
    // ручку на уровень нового send'а (а не держал старое значение).
    lastWritten = -1;
    knobShown   = -1;
}
