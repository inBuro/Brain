...
// Анти-feedback warning: выбранный параметр опасен, если это send, кормящий
// watched-шину (его id ∈ sendRefs). id-membership надёжен (не зависит от индекса).
// Шлём warn=1/0 в патч через outlet 1 (в Return он иначе не используется).
function updateFeedbackWarning(args) {
    var w = 0;
    try {
        // снят выбор параметра → нет опасности
        if (!(args && args[0] === "id" && (args[1] === 0 || args[1] === "0"))) {
            var sel = new LiveAPI("live_set view selected_parameter");
            if (sel && sel.id != 0) {
                var selId = parseInt(sel.id, 10);
                // sendRefs могут быть ещё не собраны (выбор сразу после загрузки,
                // до первого bang) → собрать on-demand, чтобы матч сработал СРАЗУ.
                if (sendRefs.length === 0) {
                    var idx = detectReturnIndex();
                    if (idx >= 0) buildRefs(idx);
                }
                for (var i = 0; i < sendRefs.length; i++) {
                    if (sendRefs[i] && sendRefs[i].id != 0 &&
                        parseInt(sendRefs[i].id, 10) === selId) { w = 1; break; }
                }
            }
        }
    } catch (e) { w = 0; }
