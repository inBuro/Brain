...
- **[[_RAW]]** — инбокс. Сюда падают входящие данные «как есть» до сортировки в проект.
...
- **`<Project Name>/`** — отдельный проект со своей структурой `_RAW/` + `wiki/`.

## Жизненный цикл данных

```
входящее → _RAW.md → распределение → <Project>/_RAW/  (сырое)
...
2. Создать `<New Project>/index.md`, `<New Project>/log.md`, `<New Project>/_RAW/assets/`, `<New Project>/wiki/{entities,concepts,sources,analysis}/`.
