...
//
// Выбор Send:
//   live.menu на лице наполняется именами return-треков (наблюдатель за
//   live_set "return_tracks"). Выбор хранится НЕ по индексу, а по ИДЕНТИЧНОСТИ
//   return-трека (его LiveAPI id + имя) — при add/remove/reorder return-ов
//   индекс пере-резолвится по сохранённому id (тот же self-healing-паттерн,
//   что у return-индекса в SF). Выбор переживает reload через pattr.
//
// Анти-feedback:
...
//   selectsend <i>      — пользователь выбрал пункт меню i (0-based по списку returns)
//   refreshmenu         — перестроить пункты меню из текущих return-треков
//   restoresel <id> <name...> — восстановить выбор из pattr на загрузке
...
var selReturnNm = "";         // имя выбранного return (для лейбла/восстановления)
...
function rebuildReturnList() {
    ...
    // Перестроить меню: clear + append каждого имени.
    outlet(2, "menu", "clear");
    for (var k = 0; k < returnNms.length; k++) {
        outlet(2, "menu", "append", returnNms[k]);
    }
...
function resolveSelection() {
    ...
    if (idx < 0) {
        // совсем пропал — откатываемся к первому
        if (returnIds.length > 0) {
            idx = 0; selReturnId = returnIds[0]; selReturnNm = returnNms[0];
        } else {
            sendIndex = -1; sendRef = null; outlet(2, "select", -1); return;
        }
    }
...
// "selectsend <i>" — пользователь выбрал пункт меню i (индекс в списке returns).
function selectsend(i) {
    i = parseInt(i, 10);
    if (isNaN(i) || i < 0 || i >= returnIds.length) return;
...
function restoresel() {
    ...
    if (String(arguments[0]) === "none") { selReturnId = -1; selReturnNm = ""; return; }
